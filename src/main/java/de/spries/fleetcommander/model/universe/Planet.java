...
public class Planet implements HasCoordinates {
	...
	public static Planet findById(Collection<Planet> planets, int planetId) {
		return planets.stream().filter(p -> p.getId() == planetId).findFirst().get();
	}

	public static Optional<Planet> filterHomePlanet(Collection<Planet> planets, Player player) {
		return planets.stream().filter(p -> p.isHomePlanetOf(player)).findFirst();
	}

	public static List<Planet> filterHomePlanets(Collection<Planet> allPlanets) {
		return allPlanets.stream().filter(Planet::isHomePlanet).collect(Collectors.toList());
	}

	public static List<Planet> filterPlayerPlanets(Collection<Planet> allPlanets, Player player) {
		return allPlanets.stream().filter(p -> p.isInhabitedBy(player))
				.collect(Collectors.toList());
	}

	public static List<Planet> filterEnemyPlanets(Collection<Planet> allPlanets, Player player) {
		return allPlanets.stream()
				.filter(p -> p.isKnownAsEnemyPlanet(player))
				.collect(Collectors.toList());
	}
